...
class Kong(AgentCheck):
    ...
    def _parse_json(self, raw, tags=None):
        ...
        output.append((self.METRIC_PREFIX + 'database.count', len(databases_metrics), tags))
        for name, items in databases_metrics:
            output.append((self.METRIC_PREFIX + 'database.items', items, tags + ['table:{}'.format(name)]))
